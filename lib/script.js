...
export const extractNewsPoints = (body) => {
  ...
  const numberedPattern = /^([1-3])[).]\s+/;
  const circlePattern = /^[①-③]\s*/;
  const ordinalPattern = /^(첫째|둘째|셋째)\s*/;

  const numberedLines = candidateLines
    .filter(
      (line) =>
        numberedPattern.test(line) ||
        circlePattern.test(line) ||
        ordinalPattern.test(line)
    )
    .map((line) =>
      cleanLine(line)
        .replace(numberedPattern, "")
        .replace(circlePattern, "")
        .replace(ordinalPattern, "")
        .trim()
    )
  ...
  const picks = [...numberedLines, ...bulletLines].slice(0, 3);
